...
def load_urdf_text(urdf_path):
    ...
    if urdf_path.suffix == ".xacro" or urdf_path.name.endswith(".urdf.xacro"):
        # Do not use launch Command() here because xacro warnings on stderr can make launch fail.
...
def generate_launch_description():
    ...
    print("[moveit_plan_to_pregrasp_preview] URDF:", urdf_path)
    print("[moveit_plan_to_pregrasp_preview] SRDF:", srdf_path)
    print("[moveit_plan_to_pregrasp_preview] kinematics:", kinematics_path)

    return LaunchDescription([
        Node(
            package="vlm_object_pointcloud_robot_tf",
            executable="moveit_plan_to_pose_cpp_node",
            name="moveit_plan_to_pose_cpp_node",
            output="screen",
            parameters=[
                robot_description,
                robot_description_semantic,
                ...
                robot_description_planning,
                {
                    "input_pose_topic": "/vlm_moveit/pregrasp_pose",

                    "planning_group": "arm",
                    "end_effector_link": "gripper_tcp",

                    # First test: position-only planning.
                    "use_position_only": True,

                    # Plan only once from the latest pregrasp pose.
                    "plan_once": True,
